  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-[#e5e7eb] hover:shadow-lg transition-all duration-300">
      <div className="mb-6">
        ...
        <div className="w-16 h-1 bg-gradient-to-r from-[#ef4444] to-[#f97316] rounded-full mx-auto mb-4"></div>

        <div className="text-center">
          <div className="text-3xl font-bold text-[#D13B00] mb-1">{outliers.length}</div>
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <ReferenceLine y={avgRateRelativity + 30} stroke="#f59e0b" strokeDasharray="5 5" />
            <ReferenceLine y={avgRateRelativity - 30} stroke="#f59e0b" strokeDasharray="5 5" />
            ...
            <Scatter name="Providers" data={data} fill="#8884d8">
              {data.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={entry.isOutlier ? severityColors[entry.severity] : "#22c55e"}
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-[#17800C]"></div>
                <span className="text-xs text-[#6b7280]">Normal Range</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-[#877F08]"></div>
                <span className="text-xs text-[#6b7280]">Medium Risk Outlier</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-[#D13B00]"></div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <div className="text-sm font-bold text-[#D13B00]">{highSeverityOutliers.length}</div>
                <div className="text-xs text-[#6b7280]">High Risk</div>
              </div>
              <div>
                <div className="text-sm font-bold text-[#877F08]">
                  {outliers.filter((d) => d.severity === "medium").length}
                </div>
                <div className="text-xs text-[#6b7280]">Medium Risk</div>
              </div>
              <div>
                <div className="text-sm font-bold text-[#A10839]">{avgRateRelativity.toFixed(0)}%</div>
