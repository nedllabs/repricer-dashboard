...
export function MedicareRateTrendChart({
  title,
  data = medicareRateTrendData.data,
}: MedicareRateTrendChartProps) {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-xs border border-[#e5e7eb] hover:shadow-lg transition-all duration-300">
      <div className="mb-6">
        ...
        <div className="w-16 h-1 bg-linear-to-r from-[#449cfb] to-[#f087fb] rounded-full mx-auto mb-4"></div>
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-[#449CFB]">
              {(data.reduce((sum, d) => sum + d.rate, 0) / data.length).toFixed(
                1
              )}
              %
            </div>
